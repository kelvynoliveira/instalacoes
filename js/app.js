...
if (toggleLegendBtn && mapLegend) {
  toggleLegendBtn.addEventListener("click", () => {
    if (mapLegend.classList.contains("legend-collapsed")) {
      ...
      toggleLegendBtn.classList.remove("hide-icon");
      mapLegend.style.display = "block";
    } else {
      mapLegend.classList.add("legend-collapsed");
      toggleLegendBtn.classList.add("hide-icon");
      setTimeout(() => {
        if (mapLegend.classList.contains("legend-collapsed")) {
    ...
    geojsonLayer = L.geoJSON(geoData, {
      ...
      onEachFeature: (feature, layer) => {
        ...
        layer.on('click', () => {
          ...
          setTimeout(() => { geojsonLayer.resetStyle(layer); }, 4000);
        });
      }
    }).addTo(map);
  })
...
campi.forEach(campus => {
  ...
  `;
  const marker = L.marker([campus.Latitude, campus.Longitude]).bindPopup(popupContent).addTo(map);
